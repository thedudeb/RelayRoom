...
- **Inline playlist creation** from both the pipeline form and the queue's manual-route action — RelayRoom calls YouTube `playlists.insert` directly so reviewers don't need to switch tabs. Verifies channel ownership before routing to existing playlists.
- **Edit-and-route** action on `needs_approval` items: change the destination playlist, override the title, override the description, then upload. Each call resolves the playlist against the connected YouTube channel so an operator can pick any playlist (or create a new one inline) without leaving the queue.
...
Google sign-in is wired through Auth.js. Set `GOOGLE_SIGNIN_CLIENT_ID`, `GOOGLE_SIGNIN_CLIENT_SECRET`, `NEXTAUTH_URL`, and `AUTH_SECRET`/`NEXTAUTH_SECRET` before testing the real Google login button. Sign-in is restricted to `INITIAL_ADMIN_EMAIL`, any comma-separated emails in `AUTH_ALLOWED_EMAILS`, and any Google Workspace domains in `AUTH_ALLOWED_DOMAINS`. For Genius org access, set `AUTH_ALLOWED_DOMAINS=geniusventuresinc.com,geniusinnovationlab.com`. `INITIAL_ADMIN_EMAIL` is promoted to the `OWNER` role the first time that account signs in. An empty email/domain allowlist **fails closed in production** — set `AUTH_ALLOW_ANY=true` (dev only, never in production) if you want to let any Google account sign in during local development.
